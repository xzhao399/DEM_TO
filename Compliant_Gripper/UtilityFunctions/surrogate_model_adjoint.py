...
class SurrogateModel_adjoint():
    ...
    def initialize_model(self):
        self.Model = ut.create_NN_model_ad(self.trainParams,mean=self.mean,var=self.var)
        ...
        # tf.keras.callbacks.ReduceLROnPlateau(monitor='total_loss',factor=0.5,min_delta=1e-2),
        _callbacks = [tf.keras.callbacks.ModelCheckpoint('DeepONet_model.h5', monitor='total_loss', save_best_only=True)]
    ...
    def trainmodel(self,verbose=False):
        for epoch in range(1,self.trainParams['Nepoch']+1):
            ...
            # Re-shuffle dataset
            self.train_ds = tf.data.Dataset.from_tensor_slices((self.trainset))
            self.train_ds = self.train_ds.shuffle(10000).batch(self.trainParams['Nbatch']) 
    ...
    def Energy_ex(self):
        ...
        _,v_out = self.Model(np.array([[120.],[36.]]).T)
        Eex = tf.squeeze(v_out*-1.)-tf.squeeze(uin*(0.5*1.*uin))-tf.squeeze(0.5*v_out*(0.005)*v_out)
        return Eex
    ...
    def Callback_EarlyStopping(self, LossList, min_delta=1e-8, patience=60):
