...
def getquadpts(L=2.,H=1.,Nelmufem=40,Nelmvfem=20,numElemU=80,numElemV=40,numGauss=2,bdcode=1):
    ...
    # inverseidx = np.arange(len(Xint[:,0]))
    # inverseidx = inverseidx[eleidx]
...
def create_NN_model(trainParams,mean=None,var=None,verbose=False):
    ...
    u_output = NN_block(x)#*(input[:,0:1]+input[:,1:2])
    v_output = NN_block(x)*(60.-input[:,1:2])/60.#*(input[:,0:1]+input[:,1:2])
...
def create_NN_model_pr(trainParams,mean=None,var=None,verbose=False):
    input = tf.keras.Input(shape=(trainParams['N_input_coord'],), name="coord")  
    x = tf.keras.layers.Normalization(mean=mean, variance=var)(input)
...
def create_NN_model_ad(trainParams,mean=None,var=None,verbose=False):
    input = tf.keras.Input(shape=(trainParams['N_input_coord'],), name="coord")  
    x = tf.keras.layers.Normalization(mean=mean, variance=var)(input)
