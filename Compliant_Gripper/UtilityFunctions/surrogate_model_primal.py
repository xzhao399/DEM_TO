...
class SurrogateModel_primal():
    ...
    def resample(self):
        ...
        rho_re = griddata(self.mesh['X_densified'].reshape((-1,2)), self.data['rho_densified'].reshape((-1,1)), (X_re[:,0], X_re[:,1]), method='nearest', fill_value=0.)
    ...
    def initialize_model(self):
        self.Model = ut.create_NN_model_pr(self.trainParams,mean=self.mean,var=self.var)
    ...
    def trainmodel(self,verbose=False):
        for epoch in range(1,self.trainParams['Nepoch']+1):
            ...
            #     ut.plotresult_contour('prediction/','solution_U.png',self.mesh['input_dof'],U_pinn,L=120,H=60,cmap='seismic')
            #     ut.plotresult_contour('prediction/','solution_V.png',self.mesh['input_dof'],V_pinn,L=120,H=60,cmap='seismic')
        self.Endepoch = epoch
    ...
    def sigma_hat(self,x,y):
        epsxx,epsxy,epsyy = self.eps_rho(x,y)
        treps = epsxx+epsyy
        # print(np.shape(epsxx))
        sigxx = self.mul2(self.material['lambda'],treps)+2*self.material['mu']*tf.squeeze(epsxx)
        sigxy = self.mul2(2*self.material['mu'],epsxy)
        sigyy = self.mul2(self.material['lambda'],treps)+2*self.material['mu']*tf.squeeze(epsyy)
        return sigxx, sigxy, sigyy
    ...
    def Energy_ex(self):
        ...
        _,vout = self.Model(np.array([[120.],[36.]]).T)
        Eex = tf.squeeze(uin*(1.-0.5*1.*uin))-tf.squeeze(0.5*vout*(0.005)*vout)
        return Eex
