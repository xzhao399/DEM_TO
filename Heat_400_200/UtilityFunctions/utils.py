...
#     error = np.linalg.norm(array1-array2)/np.linalg.norm(array1)
#     return error
...
def plotresult_contour(path,figname,X,U,cmap='jet',L=4,H=1,vmin=None,vmax=None,norm=None,nelx=80,nely=20):
    fig=plt.figure()
    ax = fig.add_subplot(111)
    u = squeeze(U)
    x = X[:,0]
...
def create_NN_model(trainParams,mean=None,var=None,verbose=False):
    ...
    def NN_block(input):
        ...
        t1 = 2.0*np.pi*input
        t1 = tf.keras.layers.Dense(trainParams['Nneuron']//2, use_bias=False, trainable=True,kernel_initializer=initializer)(t1)
        t1 = tf.concat([tf.sin(t1),tf.cos(t1)],axis=1)
        for i in range(2):
            t1 = tf.keras.layers.Dense(trainParams['Nneuron'], activation=trainParams['activation'],kernel_initializer='glorot_normal')(t1)
        t1 = tf.keras.layers.Dense(1)(t1)

        return t1
    
    T_output = NN_block(x)

    model = tf.keras.models.Model(inputs=input,outputs=T_output)
    if verbose:
        model.summary()
    return model
